...
const SEPARATOR = "======================================";
const METADATA = JSON.parse((await fs.readFile(join(dirname(fileURLToPath(import.meta.url)), "metadata.json"))).toString());
export function sendPluginChatMessage(client, ...components) {
...
export function handleCommand(sender, cmd) {
...
export function helpCommand(sender) {
    sendPluginChatMessage(sender, {
        text: SEPARATOR,
        color: "yellow",
    });
...
export function toggleParticles(sender) {
...
export async function switchServer(cmd, sender) {
    ...
    let connectionType = mode == "offline" ? ConnectType.OFFLINE : ConnectType.ONLINE, addr = ip, addrPort = Number(port);
        ...
        if (sender._onlineSession == null) {
            ...
            try {
                ...
                if (sender.state != Enums.ClientState.DISCONNECTED) {
                    sender.disconnect(Enums.ChatColor.RED +
